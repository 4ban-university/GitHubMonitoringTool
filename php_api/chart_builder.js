var repo1 = {"collaborators": {'1':'Aman Bhandal', '2':'Dmitry Kryukov', '3':'Charles-Philippe Labbe', '4':'Ksenia Popova', '5':'Nikita Baranov', '6':'Batoul Yehia', '7':'Raymart De Guzman', '8':'Andy Pham'},
...
var auth = {username: 'CharlesPhilippeLabbe', password: '1maisondu'};
var repo2 = new Repo('abhandal','SOEN341-G4', auth);

repo2.commits.then(function(commitList){
    var table = "<table><tr><th>Name</th><th>Number of commits</th><th>Percentage</th></tr>"

    var commitsPerCollaborator=commitList;
    for (var key in commitsPerCollaborator) {
        table+="<tr><td>"+key+"</td>" +
            "<td>"+commitsPerCollaborator[key]+"</td>" +
            "<td>"+Math.round(commitsPerCollaborator[key]*100/getTotal(commitsPerCollaborator))+"%</td></tr>"
    }
    table+="</table>";
...
repo2.issues.then(function(issueList){
    table = "<table><tr><th>Name</th><th>Number of issues</th><th>Percentage</th></tr>"
    var issuesPerCollaborator=issueList;

    for (var key in issuesPerCollaborator) {
        table+="<tr><td>"+key+"</td>" +
            "<td>"+issuesPerCollaborator[key]+"</td>" +
            "<td>"+Math.round(issuesPerCollaborator[key]*100/getTotal(issuesPerCollaborator))+"%</td></tr>"
    }
    table+="</table>";
    document.getElementById('issuesTable').innerHTML += table;
});

repo2.comments.then(function(commentList){
    var table = "<table><tr><th>Name</th><th>Number of comments</th><th>Percentage</th></tr>"
    var commentsPerCollaborator=commentList;

    for (var key in commentsPerCollaborator) {
        table+="<tr><td>"+key+"</td>" +
            "<td>"+commentsPerCollaborator[key]+"</td>" +
            "<td>"+Math.round(commentsPerCollaborator[key]*100/getTotal(commentsPerCollaborator))+"%</td></tr>"
    }
    table+="</table>";
...
function charts(repo){
	//var visible = true;
	overAllInfo(repo.totalCommits, repo.totalIssues, repo.collaborators);
	//Define canvas
	...
	repo.commits.then(function (commitList){
        var commitsPerCollaborator_data = commitsPerCollaboratorTransformation(commitList);
	...
	repo.comments.then(function(commentList){
        var commentsPerCollaborator_data = commentsPerCollaboratorTransformation(commentList);
	...
	repo.issues.then(function(issueList){
        var issuesPerCollaborator_data = issuesPerCollaboratorTransformation(issueList);
