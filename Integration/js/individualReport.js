repo.weeklyInfo.then(function(weeklyInfo){
	...
	for (var name in names){
		...
		individualTables(name, names[name], dataForCollaborator);
...
function individualTables(num, name, info){
	var com = 0;
		for (var key in info){
			com += info[key];
		}
...
function individualReport(data, individual_commitsPerCollaborator_chart, ctx_commitsPerCollaborator, commitsPerCollaborator_data, commitsPerCollaborator_options) {
	for (var i=0; i<data.length; i++){
		...
		//console.log(data)
		commitsPerCollaborator_data[i] = individualCommitsPerCollaboratorTransformation(data[i]);
	...
	for (var i=0; i<data.length-1; i++) {
	    ...
	    if ( this.checked ) {
	       ...
	       // document.getElementById('individual'+chartNumber+'_commitsCanvas').style.width="50%"
	        ...
	        individualCommitsPerCollaborator(chartNumber, data[chartNumber], 'bar', ctx_commitsPerCollaborator, individual_commitsPerCollaborator_chart);
	    }
	};
	document.getElementById('individual'+i+'_lineCPC').onclick = function() {
	    if ( this.checked ) {
	       ...
	       // document.getElementById('individual'+i+'_commitsCanvas').style.width="50%"
			individual_commitsPerCollaborator_chart[chartNumber].destroy();
	        individualCommitsPerCollaborator(chartNumber, data[chartNumber], 'line', ctx_commitsPerCollaborator, individual_commitsPerCollaborator_chart);
	    }
	};
	}
...
function individualCommitsPerCollaborator(num, info, chartType, ctx_commitsPerCollaborator, individual_commitsPerCollaborator_chart){
	...
	var commitsPerCollaborator_data = individualCommitsPerCollaboratorTransformation(info);
...
function individualCommitsPerCollaboratorTransformation (commitsPerCollaborator_data){
	var keyNum=0;
	var labels=[];
	var colors = '#14CCCC';
	var k;
